...
u_grid = pyvista.UnstructuredGrid(u_topology, u_cell_types, u_geometry)
#store the FE solution as Pyvista point data 
u_grid.point_data["uh"] = uex.x.array.real #.real used incase of colpex numbers 
#tell Pyvista to use "u" as active scalar visualisation: 
u_grid.set_active_scalars("uh")
...
u_error= fem.Function(V)
u_error.x.array[:] = uh.x.array - uex.x.array
...
u_grid = pyvista.UnstructuredGrid(u_topology, u_cell_types, u_geometry)
#store the FE solution as Pyvista point data 
u_grid.point_data["uh"] = u_error.x.array.real #.real used incase of colpex numbers 
#tell Pyvista to use "u" as active scalar visualisation: 
u_grid.set_active_scalars("uh")

u_plotter = pyvistaqt.BackgroundPlotter()
u_plotter.add_mesh(u_grid, show_edges=True, scalar_bar_args={"title": "Error "})
u_plotter.view_xy()
u_plotter.show_bounds(grid = True,  all_edges=True)
u_plotter.screenshot("poisson_test_case_error_plot.png")
u_plotter.show()
...
# Save as an image
screenshot_path = "solution.png"
u_plotter.screenshot(screenshot_path)

# Convert to PDF using Matplotlib
img = plt.imread(screenshot_path)
plt.imshow(img)
plt.axis("off")  # Hide axes for a clean look
plt.savefig("poisson_test_case_error.pdf", format="pdf", bbox_inches="tight", dpi=300)
