...
#Y_test = (Y_test-Y_min)/(Y_max-Y_min) dont need to normalise this?
...
plt.hist(u_X, bins=50,  alpha=0.6, label='FEniCS', color='blue')
plt.hist(u_X_DNN, bins=50, alpha=0.6, label='DNN', color='orange')
plt.xlabel('u(x) Value')
plt.ylabel('Frequency')
plt.title(f'Prior Distribution of u(x,y) at [x,y] = {coords[index]}')
plt.legend()
plt.grid(True)
plt.savefig("UQ for PDE 8.pdf", format="pdf")
