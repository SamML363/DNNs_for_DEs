...
def loss_function(theta, X, d):
    residual = residuals(theta, X, d)
    return 0.5 * np.sum(residual**2)  # MSE Loss


def numerical_gradient(theta, X, d, epsilon=1e-5):
    ...
    for i in range(len(theta)):
        ...
        num_grad[i] = (loss_function(theta_plus, X, d) - loss_function(theta_minus, X, d)) / (2 * epsilon)
    return num_grad



def gradient_check(theta, X, d, epsilon=1e-5, tol=1e-7):
    # Compute Analytical Gradient
    analytical_grad = BP(theta, X[i], d[i])
    
    # Compute Numerical Gradient
    numerical_grad = numerical_gradient(theta, X, d, epsilon)
...
gradient_check(initial_theta, X, d)
